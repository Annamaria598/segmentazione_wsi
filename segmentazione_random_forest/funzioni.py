...
def train_on_multi(
    ...
    # training di un Random Forest
    rf.fit(X, y)
    
    # test sulle immagini di test
    matthews_score, c_matrix = test_on_multiple_images(rf, test_images, test_images_segmented, window_size, feature_names, window_size_postprocessing)

    return rf, matthews_score, c_matrix
...
def image_segmentation(
    ...
    # generiamo l'immagine ottenuta dal model
    img_predetta = classes_to_colimage(y_predette, test_img.shape)
    
    # esecuzione post-processing su y ottenute dal model
