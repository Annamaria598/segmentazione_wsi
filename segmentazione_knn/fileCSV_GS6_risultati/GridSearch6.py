...
images_list = ["img/pelle303R.PNG", "img/pelle304R.PNG", "img/pelle601R.PNG"]
test_images_list = ["img/pelle301R.PNG", "img/pelle302R.PNG", "img/pelle305R.PNG"]

for s in window_size:
    # training data
    data_list = []
    for i in images_list:
        data_list.append(image_to_data(i, size=s))

    # join the dictionaries
    data = dict()
    for k in data_list[0].keys():
        data[k] = data_list[0][k].ravel()

        for d in data_list[1:]:
            data[k] = np.concatenate([data[k], d[k].ravel()])
...
seg_images_list = ["img/pelle303R_colors.PNG", "img/pelle304R_colors.PNG", "img/pelle601R_colors.PNG"]
y_list = []
for i in seg_images_list:
    y_list.append(colimage_to_classes(i))

y = np.concatenate(y_list)

# classi dei pixel per l'immagine di test
test_seg_images_list = ["img/pelle301R_colors.PNG", "img/pelle302R_colors.PNG", "img/pelle305R_colors.PNG"]
y_test_list = []
for i in test_seg_images_list:
    y_test_list.append(colimage_to_classes(i))
